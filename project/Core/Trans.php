...
class Trans{ 
	...
	function clean_filename($filename, $source_langcode = NULL) {
	  if (is_array($filename)) {
	    foreach ($filename as $key => $value) {
	      $filename[$key] = clean_filename($value, $source_langcode);
	    }
	    return $filename;
	  }
	  ...
	  $filename = self::_transliteration_process($filename, '', $source_langcode);
	  // Replace whitespace.
	  ...
	  foreach ($matches[0] as $str) {
	    ...
	    for ($i = -1; --$len; ) {
	      ...
	      if ($remaining = $tail_bytes[$c]) {
	        ...
	        $result .= self::_transliteration_replace($ord, $unknown, $source_langcode);
	        $head = '';
	      }
	      elseif ($c < "\x80") {
	        // ASCII byte.
	...
	function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
	  ...
	  if (!isset($map[$bank][$langcode])) {
	    $file = __DIR__.'/source/transliteration/' . sprintf('x%02x', $bank) . '.php';
	    if (file_exists($file)) {
	      include $file;
